...
impl MarkdownNodeType {
    pub fn head(&self) -> &'static str {
        match self {
            ...
            Self::Block(BlockNodeType::Heading(1)) => "# ",
            Self::Block(BlockNodeType::Heading(2)) => "## ",
            Self::Block(BlockNodeType::Heading(3)) => "### ",
            Self::Block(BlockNodeType::Heading(4)) => "#### ",
            Self::Block(BlockNodeType::Heading(5)) => "##### ",
            Self::Block(BlockNodeType::Heading(6)) => "###### ",
            Self::Block(BlockNodeType::Heading(_)) => "# ",
...
pub enum InlineNode {
    ...
    Italic,
    Strikethrough,
    Link(Url, Title),  // todo: swap strings for text ranges and impl Copy
    Image(Url, Title), // todo: swap strings for text ranges and impl Copy
}
...
impl PartialEq for InlineNode {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::Link(url, title), Self::Link(other_url, other_title)) => {
                url == other_url && title == other_title
            }
            (Self::Image(url, title), Self::Image(other_url, other_title)) => {
                url == other_url && title == other_title
            }
            _ => false,
        }
...
impl RenderStyle {
    pub fn apply_style(&self, text_format: &mut TextFormat, visuals: &Visuals) {
        ...
        match self {
            RenderStyle::Selection => {
                text_format.background = Color32::BLUE;
            }
            RenderStyle::PlaintextLink => {
                text_format.color = Color32::BLUE;
                text_format.underline = Stroke { width: 1.5, color: text_format.color };
            }
            RenderStyle::Syntax => {
                text_format.color = Color32::GRAY;
            }
            RenderStyle::Markdown(MarkdownNode::Document) => {
            ...
            RenderStyle::Markdown(MarkdownNode::Inline(InlineNode::Code)) => {
                text_format.background = Color32::GRAY;
                text_format.font_id.family = FontFamily::Monospace;
            ...
            RenderStyle::Markdown(MarkdownNode::Inline(InlineNode::Italic)) => {
                text_format.italics = true; // todo: font family
            }
            RenderStyle::Markdown(MarkdownNode::Inline(InlineNode::Strikethrough)) => {
                text_format.strikethrough = Stroke { width: 1.5, color: text_format.color };
            }
            ...
            RenderStyle::Markdown(MarkdownNode::Block(BlockNode::Heading(level))) => {
                ...
                match level {
                    6 => text_format.font_id.size *= 17. / 16.,
                    5 => text_format.font_id.size *= 19. / 16.,
                    4 => text_format.font_id.size *= 22. / 16.,
                    3 => text_format.font_id.size *= 25. / 16.,
                    2 => text_format.font_id.size *= 28. / 16.,
                    _ => text_format.font_id.size *= 32. / 16.,
                }
            }
