...
impl Keyboard {
    pub fn new() -> Self {
        // todo: determine scancodes based on the keyboard layout
        Self {
            keys: vec![
                ...
                Key {
                    sc: 26,
                    egui: Some(egui::Key::OpenBracket),
                    text: Some("["),
                    s_text: Some("{"),
                },
                Key {
                    sc: 27,
                    egui: Some(egui::Key::CloseBracket),
                    text: Some("]"),
                    s_text: Some("}"),
                },
    ...
    pub fn handle(
        &self, detail: u8, state: KeyButMask, pressed: bool, app: &mut WgpuLockbook,
        ...
        if pressed && (modifiers.shift_only() || modifiers.is_none()) {
            if let Some(text) = self.key_text(key, modifiers.shift) {
                app.raw_input
                    .events
        ...
        if let Some(key) = self.egui_key(key) {
    ...
    fn key_text(&self, sc: u8, shift: bool) -> Option<&'static str> {
        self.keys.iter().find(|key| key.sc == sc).and_then(|key| {
            if shift {
                key.s_text
            } else {
                key.text
    ...
    fn egui_key(&self, sc: u8) -> Option<egui::Key> {
        self.keys
            .iter()
            .find(|key| key.sc == sc)
